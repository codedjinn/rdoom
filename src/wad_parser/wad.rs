...
impl WadHeader {
    pub fn from_file(mut file: &File) -> WadHeader {
        ...
        let num_lumps: usize = convert::u8ref_to_u32(&header_raw[4..8]) as usize;
        // where to seek to for the start of the directory
        let dir_offset: usize = convert::u8ref_to_u32(&header_raw[8..12]) as usize;
    ...
    pub fn from_file(file: &mut File) -> Wad {
        ...
        if header.wad_type() == WadType::IWAD || header.wad_type() == WadType::PWAD {
            for index in 0..header.num_lumps() {
                ...
                let dir_offset: usize = convert::u8ref_to_u32(&entry_raw[0..4]) as usize;
                // the size of the lump in bytes
                let lump_size: usize = convert::u8ref_to_u32(&entry_raw[4..8]) as usize;
            ...
            for index in 0..header.num_lumps() {
                ...
                let wad_size: usize = convert::u8ref_to_u32(&entry_raw[4..8]) as usize;
                // the memory size of the lump (bytes)
                let mem_size: usize = convert::u8ref_to_u32(&entry_raw[8..12]) as usize;
