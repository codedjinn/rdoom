pub fn from_4_bytes_to_uint(arr_u8: &[u8]) -> u32 {
    if arr_u8.len() != 4 {
...
pub fn from_2_bytes_to_uint(arr_u8: &[u8]) -> u16 {
    if arr_u8.len() != 2 {
...
pub fn from_2_bytes_to_int(arr_u8: &[u8]) -> i16 {
    if arr_u8.len() != 2 {
        panic!("byte array was not of length 2");
    }

    let x1 = i16::from(arr_u8[0]);
    let x2 = i16::from(arr_u8[1]) << 8;
